...
RUN sudo apt update && sudo apt -y install apt-file && Rscript -e "if (!require('rspm')) install.packages('rspm'); rspm::enable(); BiocManager::install(c('$(echo $PKGLIST | sed 's/,/","/g')'), dependencies = c('Depends', 'Imports', 'LinkingTo', 'Suggests'))" && rm -rf /var/lib/apt/lists/*
EOF
    echo "$CONTAINER" > generated/$ID.container
  fi
else
